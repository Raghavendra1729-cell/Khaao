...
// Plex ships no Devanagari family), so only the basic-Latin subset can ever
// render a glyph here. latin-ext (Central/Eastern-European Latin) is pure
// precache weight (§ 9.12-Y1) — same reasoning as the cyrillic/greek/
// vietnamese subsets W6 already dropped (§ 9.8). Kept the same weight set as
// before (mono 500/600/700, sans 400/500/600/700) — several `font-display`
// elements carry no explicit weight class and rely on the browser's
// font-matching fallback to the nearest *registered* weight (500), so
// dropping mono-500 would silently make that text heavier, not just save
// bytes.
